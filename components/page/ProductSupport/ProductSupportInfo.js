...
function ProductSupportInfo({ pim, data }) {
	const { structure } = data
	return (
		<section id={data?.name + data?.id} className='product_support_support_box'>
			<div className='mb-17'>
				...
				<div className='mx-auto support-time'>
					...
					<p className='fs-5 text-uppercase mb-1'>
						{structure?.firstDay?.value}
					</p>
					<p className='fs-5 text-uppercase mb-8'>
						{structure?.firstTime?.value}
					</p>

					<p className='fs-5 text-uppercase mb-1'>
						{structure?.secondDay?.value}
					</p>
