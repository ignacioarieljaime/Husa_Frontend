import React from 'react'

// image
import Tv from 'public/assets/images/contact/tv.png'

function ContactUsPhoneUs() {
	return (
		<section>
			<div className='container mt-15 mb-20'>
				<h3 className='mb-15 text-center text-md-start'>Phone Us</h3>
				<div className='row justify-content-between align-items-stretch contact-info'>
					<div className='col-12 col-md-4 d-flex flex-column justify-content-between align-items-center mb-16 mb-md-0'>
						<h4 className='mb-5 align-self-md-start'>Support Hours:</h4>
						<div className='align-self-md-start'>
							<p className='fw-bold mb-1'>Monday – Friday</p>
							<p className='fw-light mb-5'>9am - 9pm EST</p>
							<p className='fw-bold mb-1'>Saturday – Sunday</p>
							<p className='fw-light mb-1'>9am - 6pm EST</p>
						</div>
					</div>
					<div className='col-12 col-md-4 d-flex flex-column justify-content-between align-items-center mb-16 mb-md-0'>
						<img src={Tv.src} alt='featured image' width='80' className='mb-5' />
						<h4 className='fs-5 mb-7'>TVs & Audio</h4>
						<p className='fw-bold fs-5 mb-0'>+1 (888) 935-8880</p>
					</div>
					<div className='col-12 col-md-4 d-flex flex-column justify-content-between align-items-center mb-16 mb-md-0'>
						<img
							src='data:image/png;base64,'
							alt='featured image'
							width='80'
							className='mb-5'
						/>
						<h4 className='fs-5 mb-7'>Appliances</h4>
						<p className='fw-bold fs-5 mb-0'>+1 (877) 465-3566</p>
					</div>
				</div>
			</div>
		</section>
	)
}

export default ContactUsPhoneUs
