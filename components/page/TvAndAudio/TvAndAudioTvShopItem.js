...
function TVAndAudioTvShopItem({ data: { image, title, link, description } }) {
	return (
		<div className='d-flex flex-column col-12 col-md-6 col-lg-3 px-6 mb-10 mb-lg-0'>
			<figure>
				<img src={image.src} alt={image.alt} className='mb-3' width='100%' />
				<figcaption>
			...
			<Link href={link.value}>
				<a className='mt-auto text-secondary text-hover-primary opacity-50 text-decoration-none fw-normal fs-5'>
					{link.title}
				</a>
			</Link>
		</div>
